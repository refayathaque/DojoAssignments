from __future__ import unicode_literals

from django.db import models

from django.shortcuts import render, HttpResponse, redirect

from django.contrib import messages

from datetime import datetime

# from django.contrib.sessions.models import Session

from ..app1.models import User

class UserManager(models.Manager):

    def make_trip(self, data):

        counter = 0
        errors = []

        # TRIP NAME
        if(len(data['destination']) < 1):
            errors.append('DESTINATION CANNOT BE BLANK!')
            return (False, errors)

        elif Trip.objects.filter(destination = data['destination']):
            errors.append('TRIP ALREADY EXISTS!')
            return (False, errors)

        else:
            counter += 1

        present = datetime.now()
        if(data['travel_start'] and data['travel_end'] < present):
            errors.append('TRIP MUST BE IN THE FUTURE!')
            return (False, errors)
        elif(data['travel_end'] < data['travel_start']):
            errors.append('TRIP END MUST BE AFTER START!')
            return (False, errors)
        else:
            counter += 1

        if counter == 2:
            Trip.objects.create(created_by = data['created_by'], destination = data['destination'], travel_start = data['travel_start'], travel_end = data['travel_end'], description = data['description'])
            return (True, data['destination']) # need session here to get username

# TABLES
class Trip(models.Model):
    destination = models.CharField(max_length=100)
    created_by = models.CharField(max_length=100)
    travel_start = models.DateField(auto_now = False)
    travel_end = models.DateField(auto_now = False)
    description = models.CharField(max_length=500)
    created_at = models.DateTimeField(auto_now_add = True)
    updated_at = models.DateTimeField(auto_now = True)
    objects = UserManager()
    def __str__(self):
        return self.destination + " " + self.travel_start + " " + self.travel_end + " " + self.description

# JOINED TABLE
class User_Trip(models.Model):
    trip_id = models.ForeignKey(Trip, related_name="trip_name")
    user_id = models.ForeignKey(User, related_name="user_name")
    created_at = models.DateTimeField(auto_now_add = True)
    updated_at = models.DateTimeField(auto_now = True)
####
